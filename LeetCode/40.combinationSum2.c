...
void backtracking(int *candidates, int candidatesSize, int target, int index, int **out, int *returnSize, int **returnColumnSizes){
    ...
    if(pathSum>=target){// 停止条件
        if(pathSum==target){
            ...
            //     printf("%d ", path[i]);
            // }printf("\n");
            out[(*returnSize)]=(int *)malloc(sizeof(int)*100);
    ...
    for(i=index;i<candidatesSize;i++){
        ...
        pathTop++;
        backtracking(candidates, candidatesSize, target, i+1, out, returnSize, returnColumnSizes);
        pathSum-=candidates[i];
        pathTop--;
    }
    return ;
...
int main(){
    int i, j;
    int **out;
    int candidates[10]={10, 1, 2, 7, 6, 1, 5};
    int target=8;
    ...
    *returnSize=0;
    out=combinationSum2(candidates, candidatesSize, target, returnSize, returnColumnSizes);
